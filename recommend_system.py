...
# Определяем путь к файлам в зависимости от того, в каком режиме запущена программа
if getattr(sys, 'frozen', False):
    # Если программа была собрана с помощью PyInstaller
    application_path = sys._MEIPASS
else:
    # Если программа не собрана, то путь будет указывать на текущую директорию
    application_path = os.path.dirname(__file__)

# Путь к файлам
dataset_path = os.path.join(application_path, 'id-name.csv')
games_path = os.path.join(application_path, 'games_ym_raz.csv')
model_path = os.path.join(application_path, 'nearest_neighbors_model.pkl')
